 ...
 */
@Data
@ApiModel(value = "流程模版", description = "流程模版")
public class TmplProcess extends BaseModel {
    ...
    /** 流程模版名称 */
    @ApiModelProperty(value="流程模版名称", name="name", dataType="String", example="动力学院专利提案模版")
    private String name;
    ...
    public String checkValue(){
        ...
        if (this.getOrganizationId() == null){
            return "TMPLPROCESS_ORGANIZATION_ID_CANNOT_NULL";
        }
        if (StringUtils.isNullOrEmpty(this.getName())){
            return "TMPLPROCESS_NAME_CANNOT_NULL";
        }
        if (this.getName() == null){
            return "TMPLPROCESS_NAME_CANNOT_NULL";
        }
        if (StringUtils.stringCount(this.getName()) > 255){
            return "TMPLPROCESS_NAME_MAX_LENGTH_ERROR";
        }
        if (StringUtils.isNullOrEmpty(this.getType())){
            return "TMPLPROCESS_TYPE_CANNOT_NULL";
        }
        if (this.getType() == null){
            return "TMPLPROCESS_TYPE_CANNOT_NULL";
        }
        if (StringUtils.isNullOrEmpty(this.getIsDelete())){
            return "TMPLPROCESS_IS_DELETE_CANNOT_NULL";
        }
    ...
    @Override
    public TmplProcess setDefaultValue(){
