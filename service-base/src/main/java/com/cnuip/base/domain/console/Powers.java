 ...
 */
@Data
@ApiModel(value = "职权表", description = "职权表")
public class Powers extends BaseModel {
    ...
    /** 名称 */
    @ApiModelProperty(value="名称", name="name", dataType="String", example="职权一")
    private String name;

    /** 备注 */
    @ApiModelProperty(value="备注", name="remark", dataType="String", example="职权一xxxx")
    ...
    public String checkValue(){
        ...
        if (this.getOrganizationId() == null){
            return "POWERS_ORGANIZATION_ID_CANNOT_NULL";
        }
        if (StringUtils.isNullOrEmpty(this.getName())){
            return "POWERS_NAME_CANNOT_NULL";
        }
        if (this.getName() == null){
            return "POWERS_NAME_CANNOT_NULL";
        }
        if (StringUtils.stringCount(this.getName()) > 255){
            return "POWERS_NAME_MAX_LENGTH_ERROR";
        }
        if (StringUtils.stringCount(this.getRemark()) > 255){
            return "POWERS_REMARK_MAX_LENGTH_ERROR";
        }
        if (StringUtils.isNullOrEmpty(this.getIsDelete())){
            return "POWERS_IS_DELETE_CANNOT_NULL";
        }
